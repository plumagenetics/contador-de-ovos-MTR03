...
if cfg["executar"]:
    ...
    if not linhas_brutas:
        st.error("Nenhuma linha vÃ¡lida encontrada no PDF.")
        st.stop()

    st.session_state.linhas_brutas = linhas_brutas
    status_callback(f"{len(linhas_brutas)} linhas extraÃ­das do PDF.")

    status_callback("Processando linhas (prÃ©-processamento)...")
    ...
    status_callback("CÃ¡lculo concluÃ­do.")
